...
namespace AFIT_Cooperative.Helper
{
    public class JwtToken
    {
        ...
        {
            Member decodedToken = new Member();
            var v = handler.ReadJwtToken(token).Payload.First().Value.ToString();
            decodedToken = JsonConvert.DeserializeObject<Member>(v);
            return decodedToken;
        }
    ...
    public class Auth
    {
        ...
        public static Member ValidateUser(Member member)
        {
            return _context.Members.SingleOrDefault((u) => u.Id == member.Id);
        }
